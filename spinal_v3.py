...
import subprocess
import shutil
from scipy.ndimage import center_of_mass, label, binary_fill_holes, binary_opening, binary_closing
...
def extract_vertebral_body_by_intensity(ct_slice: np.ndarray, vertebra_mask: np.ndarray) -> np.ndarray:
    """
    Extract vertebral body based on CT intensity (gray trabecular bone area).
    
    In CT axial view:
    - White areas: Cortical bone (~400-1000+ HU)
    - Gray areas: Trabecular/cancellous bone (~100-400 HU) - THIS IS THE VERTEBRAL BODY
    - Dark areas: Soft tissue, air
    
    Args:
        ct_slice: 2D CT intensity values at one z-level
        vertebra_mask: 2D binary mask of full vertebra at same z-level
        
    Returns:
        2D binary mask of vertebral body (trabecular bone region) only
    """
    if np.sum(vertebra_mask > 0) == 0:
        return vertebra_mask
    
    masked_ct = ct_slice.copy()
    masked_ct[vertebra_mask == 0] = -1000
    
    trabecular_min = 100
    trabecular_max = 500
    
    trabecular_mask = (masked_ct >= trabecular_min) & (masked_ct <= trabecular_max) & (vertebra_mask > 0)
    dense_bone_mask = (masked_ct > trabecular_max) & (masked_ct <= 800) & (vertebra_mask > 0)
    
    bone_interior = trabecular_mask | dense_bone_mask
    
    bone_interior = binary_closing(bone_interior, iterations=2)
    bone_interior = binary_fill_holes(bone_interior)
    bone_interior = binary_opening(bone_interior, iterations=1)
    
    labeled, num_features = label(bone_interior)
    
    if num_features == 0:
        return vertebra_mask.astype(np.float32)
    
    component_sizes = []
    for i in range(1, num_features + 1):
        component_sizes.append((i, np.sum(labeled == i)))
    
    component_sizes.sort(key=lambda x: x[1], reverse=True)
    
    vertebral_body = (labeled == component_sizes[0][0]).astype(np.float32)
    vertebral_body = binary_fill_holes(vertebral_body).astype(np.float32)
...
def process_cervical_spine(ct_path: str, target_vertebra: str = 'C1', sagittal_slice: int = None) -> str:
    ...
    data = img.get_fdata()
    voxel_spacing = img.header.get_zooms()  # (x, y, z) spacing in mm
    pixel_area_mm2 = voxel_spacing[0] * voxel_spacing[1]  # Area of one pixel in mm²
    ...
    # ROW 3: Vertebral Body Identification (from vertebral_body_axial.py logic)
    axial_ct_raw = data[:, :, target_z]
    axial_seg = target_seg[:, :, target_z]
    vertebral_body = extract_vertebral_body_by_intensity(axial_ct_raw, axial_seg)
    
    # LEFT: Vertebral body only - masked CT
    body_mask = vertebral_body.T.copy().astype(float)
    body_mask[body_mask == 0] = np.nan
    
    masked_ct = axial_ct.copy()
    masked_ct[np.isnan(body_mask) | (body_mask == 0)] = -1000
    
    axes[2, 0].imshow(masked_ct, cmap='gray', origin='lower', vmin=-400, vmax=1000)
    axes[2, 0].set_title(f'{target_vertebra} Vertebral Body Only (Gray Trabecular Bone)', fontsize=14)
    axes[2, 0].axis('off')
    
    # RIGHT: Vertebral body highlighted with red overlay
    axes[2, 1].imshow(axial_ct, cmap='gray', origin='lower', vmin=-400, vmax=1000)
    
    body_overlay = np.zeros((*axial_ct.shape, 4))
    body_bool = ~np.isnan(body_mask) & (body_mask > 0)
    body_overlay[body_bool, 0] = 1.0
    body_overlay[body_bool, 1] = 0.2
    body_overlay[body_bool, 2] = 0.0
    body_overlay[body_bool, 3] = 0.5
    axes[2, 1].imshow(body_overlay, origin='lower')
    
    body_coords = np.where(vertebral_body > 0)
    if len(body_coords[0]) > 0:
        body_area_pixels = np.sum(vertebral_body > 0)
        body_area_mm2 = body_area_pixels * pixel_area_mm2  # Convert to mm²
        body_x_min, body_x_max = body_coords[0].min(), body_coords[0].max()
        body_y_min, body_y_max = body_coords[1].min(), body_coords[1].max()
        body_width = body_x_max - body_x_min
        body_height = body_y_max - body_y_min
        body_centroid_x = int(np.mean(body_coords[0]))
        body_centroid_y = int(np.mean(body_coords[1]))
        
        rect = plt.Rectangle((body_x_min, body_y_min), body_width, body_height,
                              fill=False, edgecolor='lime', linewidth=2, linestyle='--')
        axes[2, 1].add_patch(rect)
        
        # Place label beside the vertebral body (to the right), not on top
        axes[2, 1].annotate(f'{target_vertebra}\nVertebral Body\nArea: {body_area_mm2:.1f} mm²', 
                            xy=(body_x_max, body_centroid_y),  # Arrow points to right edge of body
                            xytext=(body_x_max + 40, body_centroid_y),  # Text placed to the right
                            fontsize=12, fontweight='bold',
                            color='white', ha='left', va='center',
                            bbox=dict(boxstyle='round,pad=0.3', facecolor='red', alpha=0.8),
                            arrowprops=dict(arrowstyle='->', color='lime', lw=2))
        
        print(f"{target_vertebra} Vertebral Body: area={body_area_mm2:.1f} mm² ({body_area_pixels} pixels), size={body_width}x{body_height}")
    
    axes[2, 1].set_title(f'{target_vertebra} Vertebral Body Identified', fontsize=14)
    axes[2, 1].axis('off')

    # ROW 4: All C1-C7 vertebrae labeled
    ...
    axes[3, 0].imshow(sagittal_all_ct, cmap='gray', origin='lower', vmin=-400, vmax=1000)
    ...
    for vert, vert_seg in vertebrae_data.items():
        ...
        axes[3, 0].imshow(vert_overlay, origin='lower')
        
        # Add label at centroid y, z position
        cy, cz = vertebrae_centroids[vert][1], vertebrae_centroids[vert][2]
        axes[3, 0].annotate(vert, (cy, cz), fontsize=12, fontweight='bold',
                            color='white', ha='center', va='center',
                            bbox=dict(boxstyle='round,pad=0.2', facecolor=colors[vert], alpha=0.8))
    
    axes[3, 0].set_title(f'All Cervical Vertebrae (C1-C7) - Sagittal View (x={avg_x})', fontsize=14)
    axes[3, 0].axis('off')
    
    # RIGHT: Legend and info
    axes[3, 1].axis('off')
    legend_y = 0.9
    axes[3, 1].text(0.5, 0.95, 'Vertebrae Legend', fontsize=16, fontweight='bold',
                    ha='center', va='top', transform=axes[3, 1].transAxes)
    ...
    for i, vert in enumerate(all_vertebrae):
        ...
        rect = plt.Rectangle((0.1, y_pos - 0.03), 0.08, 0.06, 
                            facecolor=color, transform=axes[3, 1].transAxes, clip_on=False)
        axes[3, 1].add_patch(rect)
        axes[3, 1].text(0.22, y_pos, f'{vert}: {status}', fontsize=12,
                        ha='left', va='center', transform=axes[3, 1].transAxes)
